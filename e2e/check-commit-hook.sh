...
original_content=$(cat e2e/precommit-e2e.test)
echo "Original content: $original_content"

# Run the hook
python find_and_replace_strings/main.py --config e2e/.find-and-replace.json e2e/precommit-e2e.test

# Check if the expected changes have been made
content=$(cat e2e/precommit-e2e.test)
...
echo "$original_content" > e2e/precommit-e2e.test
